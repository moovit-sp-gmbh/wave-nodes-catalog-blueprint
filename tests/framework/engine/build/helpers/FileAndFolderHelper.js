...
async function _copyFile(srcPath, destPath, progressCallback, getSrcChecksum, signal) {
    await createDestFolderIfNotExists(path_1.default.dirname(destPath));
...
async function _copyFolder(srcPath, destPath, progressCallback, getSrcChecksum, abortSignal) {
    ...
    if (progressCallback && progressData.lastEmittedPercent < 100) progressCallback(100);
    ...
    for (const entry of entries) {
        ...
        if (fileStat.isDirectory()) {
            ...
            if (!progressCallback) {
                ...
                const checksum = await _copyFile(
                    srcPath,
                    destPath,
                    (filePercent) => {
...
async function _getFileMd5Hash(filePath, signal) {
    return new Promise((resolve, reject) => {
        ...
        const cleanup = () => {
            if (signal) signal.removeEventListener("abort", destroyStream);
        };
        if (signal) {
            signal.addEventListener("abort", destroyStream);
        }
        fileStream.on("data", (data) => {
            if (!aborted) hash.update(data);
        });
        fileStream.on("end", () => {
            cleanup();
...
async function getFilesRecursively(directory) {
    ...
    const files = entries
        ...
        .map((file) => path_1.default.resolve(path_1.default.join(directory, file.name)));
...
async function createDestFolderIfNotExists(folderPath) {
    try {
