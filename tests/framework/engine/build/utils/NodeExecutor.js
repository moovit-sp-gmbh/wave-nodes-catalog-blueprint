...
class NodeExecutor {
    ...
    async process(dry, executionStateHelper, runner, additionalConnectorRoot, catalogPath, extraCatalogLocations) {
        ...
        try {
            ...
            if (!nextNode.bypass) {
                const node = await (0, NodeImport_1.requireNodeByPath)(
                    nextNode,
                    catalogPath,
                    additionalConnectorRoot ? true : false,
                    extraCatalogLocations,
                    this.executionPackage.hcl.getAuthToken()
                );
                ...
                try {
                    ...
                    if (nodeResult.inputs) {
                        ...
                        if (errorIndex !== -1) {
                            throw new StreamNodeErrors_1.StreamWildcardReplacerError(
                                new Error("Node preparation failed"),
                                nodeResult.inputs[errorIndex].errorMessage
                            );
                        }
                    }
    ...
    async prepare(catalogPath, extraCatalogLocations, isAdditionalConnector = false) {
        ...
        try {
            ...
            if (this.result.inputs && this.result.inputs.find((input) => input.error)) {
                ...
                throw new StreamNodeErrors_1.StreamWildcardReplacerError(
                    new Error("Failed to resolve wildcard(s)"),
