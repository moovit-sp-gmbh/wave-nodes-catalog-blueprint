...
async function findAgent() {
	if (process.platform === "darwin") {
		const execPath = process.env.HCLOUD_EXEC_PATH || DEFAULT_LOCATIONS.MAC.EXEC
		const configLocation = path.join(execPath, "Contents", "Resources", "agent-config.yaml")
		if (!await fs.stat(configLocation).then(() => true).catch(() => false)) {
		...
		if (!await fs.stat(localConfigLocation).then(() => true).catch(() => false)) {
			throw new Error(`local config file not in ${localConfigLocation}. Make sure to run the agent at least once`)
		}
		...
		const execPath = process.env.HCLOUD_EXEC_PATH || DEFAULT_LOCATIONS.WINDOWS.EXEC
		const configLocation = path.join(execPath, "agent-config.yaml")
		if (!await fs.stat(configLocation).then(() => true).catch(() => false)) {
			throw new Error(`config file not in ${configLocation}`)
		}

		const homePath = process.env.HCLOUD_HOME || DEFAULT_LOCATIONS.WINDOWS.HOME
		const localConfigLocation = path.join(homePath, "config.local.yaml")
		if (!await fs.stat(localConfigLocation).then(() => true).catch(() => false)) {
			throw new Error(`local config file not in ${localConfigLocation}. Make sure to run the agent at least once`)
		}
		...
		throw new Error("debugging on this operating system is currently not supported")
	}
}
